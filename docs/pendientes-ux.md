...
Última actualización: **2026-09-05** (tooltip en el "+" de agregar otro documento de ejemplo, en Calibración).
...
| **Aviso "Recomendación antes de activar"** | `RecomendacionEjemplos.svelte`, disparado al prender el switch "Ejemplo documental" | 2026-09-04, a pedido explícito con mockup compartido | Prender el switch (no apagarlo — eso sigue siendo directo, sin aviso) ahora revisa primero cuántas instancias de documento de ejemplo tiene el tipo (`tipo.documentosEjemplo.length`, cuenten o no ya con recortes hechos). Con menos de `MINIMO_EJEMPLOS_RECOMENDADO` (3, copia tal cual del mockup) se abre este aviso EN VEZ de activar directo; con 3 o más, activa igual que antes, sin aviso. Dos acciones: **"Agregar más ejemplos"** cierra el aviso y navega a Calibración (`abrirCalibracion`) SIN activar el switch; **"Continuar con la activación"** cierra el aviso Y SÍ lo activa, ignorando la recomendación. Es un `Dialog` normal, no un `AlertDialog` como `ConfirmarAccion`: Escape y el clic fuera lo cierran sin forzar ninguna de las dos respuestas, porque es una recomendación blanda, no una confirmación antes de algo destructivo. El layout (ícono centrado en cuadro `bg-primary/10`, título y cuerpo centrados, pregunta en negritas, botones alineados a la DERECHA) es distinto a propósito del molde usual de `ConfirmarAccion` (ícono + texto en fila): así lo trae el mockup compartido. Sigue sin haber ningún envío real a Document AI detrás de nada de esto — verificado que la marca de "listo" en cualquiera de los dos caminos no dispara ninguna llamada de red. **Fidelidad con Figma confirmada el mismo día, ronda aparte**: el mockup resultó ser un frame real del archivo (`HU001 | 16`, node `1294:58486`) — se pudo pedir vía MCP en vez de aproximar a ojo. Se copiaron literales: padding 24px, gap 32px entre bloques (8px entre ícono/texto/botones) y 16px entre título/cuerpo/pregunta, radio 12px del contenedor y 8px del ícono, tamaños de fuente 24px (título, antes 18px) y 16px (cuerpo y pregunta, antes 14px), sombra de dos capas, borde de 2px (antes 1px), y que los botones van a la derecha (`justify-end`, antes centrados). Los colores literales del frame (`#55bfe7` de acento, `#e4f3fc` del ícono) NO se copiaron — son de una paleta vieja que ya no corresponde al azul real de la app (`--primary: #4268fb`); se usaron los tokens propios, que además resultaron ser un match EXACTO para los grises de texto (`#1f2937` y `#6b7280`) sin necesitar ningún hex nuevo. El ancho (`max-w-lg`, sin dato literal del frame — MCP se quedó sin cuota antes de poder pedir las dimensiones exactas) se ajustó a ojo hasta que el párrafo envolviera en 3 líneas como en la captura de referencia. |
| **El recorte guardado arranca REPLEGADO** | Cada campo con `ejemplo`, pantalla de Calibración | 2026-09-04, a pedido explícito | Antes, en cuanto un campo tenía un recorte guardado, "Texto seleccionado" + la imagen se mostraban siempre — con varios campos ya recortados la pantalla crecía rápido. Ahora arrancan plegados: junto al lápiz de "Editar" y la basura aparece un tercer botón, un chevron (hacia abajo cuando está plegado, hacia arriba cuando no) que muestra/oculta ese bloque. El estado de plegado/desplegado (`camposExpandidos`, un `Set` de nombres de campo) es puramente de PANTALLA — no se guarda en `localStorage`, y no se resetea al cambiar de instancia de documento en el selector de chips: es una preferencia de la sesión, no del dato. **Incidente real al construirlo**: la primera versión usaba `let camposExpandidos = $state<Set<string>>(new Set())` — picar el chevron no hacía nada, porque `$state` en Svelte 5 NO intercepta los métodos mutantes (`.add()`/`.delete()`) de un `Set`/`Map` nativo, solo la reasignación de la variable. Se corrigió con `SvelteSet` de `svelte/reactivity` (`let camposExpandidos = new SvelteSet<string>()`, sin envolver en `$state`, ya reactivo por sí solo) — detectado por una prueba que picaba el chevron y comprobaba que la imagen apareciera, no por inspección visual (se hubiera visto igual de "plegado" con o sin el bug). |
| **"Recortar" ya NO alterna — solo ACTIVA** | Modal de recorte (`RecortarEjemploCampo.svelte`) | 2026-09-05, a pedido explícito | Antes, con el modo recorte ya activo (que arranca activo por default desde el 2026-09-04), picar "Recortar" de nuevo lo APAGABA y borraba el rectángulo — un verdadero toggle. Ahora `activarRecorte()` (antes `alternarRecorte()`) solo pone `modoRecorte = true`; picarlo con el modo ya activo no hace nada, ni al rectángulo ni al estado. "Cancelar" sigue siendo el ÚNICO camino para apagar el modo (y sí borra el rectángulo, sin cambios ahí) — tras cancelar, "Recortar" vuelve a servir para reactivarlo. **Esto no es solo un cambio de gusto: arregla un bug real** — con un rectángulo PRECARGADO por "Editar" (2026-09-04), un clic de más en "Recortar" (fácil de dar sin querer, ya que el botón se ve igual de "presionado" con el modo ya activo) borraba silenciosamente la precarga. Verificado con una prueba dedicada que reproduce exactamente ese caso. |
| **Tooltip en el "+" de agregar otro documento** | Selector de instancias, Calibración | 2026-09-05, a pedido explícito | El ícono "+" (sin texto, solo visible cuando ya hay al menos una instancia — ver la fila de "varias instancias" arriba) ahora explica su función al pasar el mouse: "Puedes agregar más ejemplos documentales." Mismo patrón `Tooltip.Provider`/`Root`/`Trigger`/`Content` ya usado en el resto del módulo. |
