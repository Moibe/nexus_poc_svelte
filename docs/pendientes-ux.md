...
Última actualización: **2026-09-03** ("Guardar" del recorte ahora cierra el modal y regresa a "Calibración").
...
| **"Listar versiones anteriores"**, junto a "Modelos documentales agregados" | Columna derecha de la Biblioteca | 2026-09-02 | **Sin frame de Figma.** Solo aparece con un tipo seleccionado en el árbol que además tenga `historialVersiones.length > 0` — nada de mostrar un link sin efecto. Al picarlo, debajo de la tarjeta vigente aparecen tarjetas de solo lectura (fondo punteado, sin botones ni menú) por cada versión anterior, más reciente primero: nombre, insignia `0.0.N`, cuántos campos tenía y cuándo se publicó. El toggle se apaga solo al cambiar de selección (`$effect` sobre `seleccionadoId`), para no arrastrar el historial de un modelo a la tarjeta de otro. Requirió agregar `historialVersiones` y `activadoEn` a `TipoDocumentalGuardado` — **hoy la app no guardaba ningún rastro de una versión reemplazada** (el `procesadorId` viejo simplemente se sobreescribía). Un tipo que ya se re-publicó ANTES de este cambio no tiene cómo recuperar ese historial retroactivamente; desde ahora sí queda. El detalle fino de CÓMO se archiva —por qué no basta con leer `tipo.campos` en el momento en que el back responde— está en el comentario de `versionEnEdicion` en `configuracion.svelte.ts`: para cuando `activarTipoDocumental` se entera del éxito, los campos ya se sobreescribieron con lo recién editado, así que `crearNuevaVersion()` toma la foto ANTES de tocar nada. |
