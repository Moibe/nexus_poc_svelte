...
| **Las ramas del árbol FILTRAN la lista** | Submenú de la Biblioteca | Cambio pedido (2026-08-28): picar una rama deja visible solo su tarjeta; picarla de nuevo, o picar "Biblioteca" (el padre), vuelve a mostrar todas. La rama seleccionada se pinta en el color primario y expone `aria-pressed`. Retomar el modelo quedó SOLO en la tarjeta — un mismo gesto no debe filtrar y navegar a la vez. El filtro es efímero: entrar al wizard o cerrar el módulo lo limpia, para que el aviso verde de un alta nunca conviva con un filtro que esconde al recién nacido. |
