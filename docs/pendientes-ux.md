...
| **Un modelo ACTIVO se bloquea para edición** | Wizard del Módulo de configuración | Cambio pedido (2026-09-01): al abrir un tipo `activo`, todo el wizard queda de solo lectura (los tres pasos) con un candado ámbar que explica por qué y ofrece "Crear nueva versión" — la ÚNICA puerta para volver a editarlo. Implementa la regla que ya estaba anotada para el mapeo en `nexus_back/docs/solicitudes-dba.md` §2.6b ("corregir obliga a versión nueva, no hay atajo"), generalizada a TODOS los campos: antes, picar la tarjeta de un modelo activo abría un wizard editable de verdad, y los cambios se guardaban en local sin avisar mientras Document AI seguía sirviendo la configuración vieja — divergencia silenciosa. "Crear nueva versión" no toca Google: solo regresa el registro a `borrador`; el procesador sigue sirviendo lo publicado hasta la próxima activación. |
| **El botón dice "Publicar cambios" en vez de "Activar"** | Tarjeta de la Biblioteca | Solo cuando `version > 0` (o sea, ya se publicó antes). Llamarle "Activar" a la segunda publicación de algo que ya estuvo vivo sugiere que nunca lo estuvo. |
